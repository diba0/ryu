...
class simple_Awareness(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        # # Install tcp syn flow entry
        # match_syn = parser.OFPMatch(eth_type=0x0800, ip_proto=6, tcp_flags=0x02)  # TCP SYN
        # actions_syn = [parser.OFPActionOutput(ofproto.OFPP_CONTROLLER, ofproto.OFPCML_NO_BUFFER)]
        # self.add_flow(datapath, 2, match_syn, actions_syn)
    ...
    def show_topology(self):
        ...
        nx.draw(self.graph)
        print(f"nodes: {self.graph.nodes()}; links: {self.graph.edges()}")
        # plt.show()
        # plt.savefig("/home/ubuntu/ryu/ryu/app/RSIR-Reinforcement-Learning-and-SDN-Intelligent-Routing/SDNapps_proac/%d.png" % int(time.time()))

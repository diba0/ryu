...
def get_paths_base():
    file_base = ('/home/ubuntu/ryu/ryu/app/RSIR-Reinforcement-Learning-and-SDN-Intelligent-Routing/SDNapps_proac'
                 '/RoutingGeant/paths_weight.json')
...
def get_paths_RL():
    file_RL = ('/home/ubuntu/ryu/ryu/app/RSIR-Reinforcement-Learning-and-SDN-Intelligent-Routing/SDNapps_proac/paths'
               '.json')
...
def stretch(paths, paths_base, src, dst):
    if isinstance(paths.get(str(src)).get(str(dst))[0], list):
        ...
        add_stretch = float(len(paths.get(str(src)).get(str(dst))[0])) - float(
            len(paths_base.get(str(src)).get(str(dst))))
        mul_stretch = float(len(paths.get(str(src)).get(str(dst))[0])) / float(
            len(paths_base.get(str(src)).get(str(dst))))
...
def calc_all_stretch(cont):
    ...
    a = time.time()
    with open(
            '/home/ubuntu/ryu/ryu/app/RSIR-Reinforcement-Learning-and-SDN-Intelligent-Routing/SDNapps_proac'
            '/RoutingGeant/stretch/' + str(
                cont) + '_stretch.csv', 'w') as csvfile:
...
def RL_thread():
    ...
    while cont < 836:
        ...
        a = time.time()
        data = pd.read_csv(
            "/home/ubuntu/ryu/ryu/app/RSIR-Reinforcement-Learning-and-SDN-Intelligent-Routing/SDNapps_proac/net_info"
            ".csv")

...
class simple_Monitor(app_manager.RyuApp):
        ...
        for stat in sorted(body, key=attrgetter('port_no')):  # get the value of port_no form body
            ...
            if port_no != ofproto_v1_3.OFPP_LOCAL:
                if port_no != 1 and self.awareness.link_to_port:
                    ...
                    file = '/home/ubuntu/ryu/ryu/app/RSIR-Reinforcement-Learning-and-SDN-Intelligent-Routing/OSPF_loss/Proac/bw_r.txt'  # random link capacities
    ...
    def port_status_handler(self, ev):
        ...
        if reason in reason_dict:
            print
            "switch%d: port %s %s" % (dpid, reason_dict[reason], port_no)
        else:
            print
            "switch%d: Illegal port state %s %s" % (dpid, port_no, reason)
        ...
        if self.delay.link_delay:
            for link in self.link_free_bw:
                ...
                self.net_metrics[link] = [round(self.link_free_bw[link], 6), round(self.link_used_bw[link], 6),
                                          round(self.link_loss[link], 6), round(self.delay.link_delay[link], 6)]

            file_metrics = '/home/ubuntu/ryu/ryu/app/RSIR-Reinforcement-Learning-and-SDN-Intelligent-Routing/OSPF_loss/Proac/Metrics/' + str(
